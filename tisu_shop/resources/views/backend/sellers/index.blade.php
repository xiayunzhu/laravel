            ...
            <div class="layui-form-item">
                <div class="layui-inline">
                    <div class="layui-input-inline">
                        <input type="text" class="layui-input"  name="name" placeholder="昵称">
                    </div>
                </div>
                <div class="layui-inline">
                    <div class="layui-input-inline">
                        <input type="text" class="layui-input"  name="phone" placeholder="手机号">
                    </div>
                </div>
                <div class="layui-inline">
                    <div class="layui-input-inline">
...
@push('scripts')
    <script>
        ...
        function deleteRow(id, obj) {
            let url = "{{ url('admin/sellers/destroy') }}/" + id;
        ...
        function deleteRows(ids) {
            ...
            let url = "{{ url('admin/sellers/destroyBat') }}";
            let data = {ids: ids};
        ...
        layui.use(['table', 'form'], function () {
            ...
            table.render({
                elem: '#tbl'
                // , height: 312
                , url: "{{route('admin.sellers.list')}}"  //数据接口
                , title: '用户列表'
                , page: true //开启分页
                ...
                , cols: [[ //表头
                    ...
                    , {
                        field: 'login_at', title: '上次登录时间', templet: function (row) {
            ...
            table.on('toolbar(tblFilter)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.msg('添加');
                        window.location.href = "{{ route('admin.sellers.create') }}";
                        break;
                    case 'update':
                        if (data.length === 0) {
                            layer.msg('请选择一行');
                            ...
                            window.location.href = "{{ url('admin/sellers/edit') }}/" + checkStatus.data[0].id;
                        }
                        break;
                    case 'delete':
                        if (data.length === 0) {
            ...
            table.on('tool(tblFilter)', function (obj) { //注：tool 是工具条事件名，tblFilter 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'password') {
                    ...
                    window.location.href = "{{ url('admin/sellers/password') }}/" + data.id;
                    ...
                    //跳转到编辑页面 admin.sellers.edit
                    window.location.href = "{{ url('admin/sellers/edit') }}/" + data.id;
                }
            });
            //监听查询按钮
