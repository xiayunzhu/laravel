        ...
        <div class="layui-form">
            <div class="layui-form-item">
                ...
                <div class="layui-inline">
                    <div class="layui-input-inline">
                        <div class="layui-input-inline">
                            <input type="text" class="layui-input" name="address" placeholder="登录地址">
                        </div>
                    </div>
                </div>
                <div class="layui-inline">
...
@push('scripts')
    <script>
        ...
        function deleteRow(id, obj) {
            let url = "{{ url('admin/loginLogs/destroy') }}/" + id;
        ...
        function deleteRows(ids) {
            ...
            let url = "{{ url('admin/loginLogs/destroyBat') }}";
            let data = {ids: ids};
        ...
        layui.use(['table', 'form'], function () {
            ...
            table.render({
                elem: '#tbl'
                // , height: 312
                , url: "{{route('admin.loginLogs.list')}}"  //数据接口
                , title: "{{$title}}"
                , page: true //开启分页
                ...
                , cols: [[ //表头
                    ...
                    , {field: 'address', title: '登录地址', width: 180}
                    , {
                        field: 'login_time', title: '登录时间', width: 180, templet: function (row) {
                            return dateFormt(row.login_time);
                        }
                    }
                    , {field: 'user_name', title: '登陆人', width: 100}
                    , {field: 'browser', title: '浏览器', width: 160}
                    , {field: 'device', title: '设备名称', width: 120}
                    , {field: 'device_type', title: '设备类型', width: 120}
                    , {field: 'ip', title: '登录IP', width: 120}
                    , {field: 'language', title: '语言'}

                    , {field: 'platform', title: '操作系统', width: 120}
            ...
            table.on('toolbar(tblFilter)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.msg('添加');
                        window.location.href = "{{ route('admin.loginLogs.create') }}";
                        break;
                    case 'update':
                        if (data.length === 0) {
                            layer.msg('请选择一行');
                            ...
                            window.location.href = "{{ url('admin/loginLogs/edit') }}/" + checkStatus.data[0].id;
                        }
                        break;
                    case 'delete':
                        if (data.length === 0) {
            ...
            table.on('tool(tblFilter)', function (obj) { //注：tool 是工具条事件名，tblFilter 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') {
                    ...
                    //跳转到编辑页面
                    window.location.href = "{{ url('admin/loginLogs/edit') }}/" + data.id;
                }
            });
