...
@push('scripts')
    <script>
        ...
        function deleteRow(id, obj) {
            let url = "{{ url('admin/uploadFiles/destroy') }}/" + id;
        ...
        function deleteRows(ids) {
            ...
            let url = "{{ url('admin/uploadFiles/destroyBat') }}";
            let data = {ids: ids};
        ...
        layui.use(['table','form'], function () {
            ...
            table.render({
                elem: '#tbl'
                // , height: 312
                , url: "{{route('admin.uploadFiles.list')}}"  //数据接口
                , title: "{{$title}}"
                , page: true //开启分页
                ...
                , cols: [[ //表头
                    ...
                    , {field: 'folder', title: '文件对象类型', width: 120}
                    , {field: 'object_id', title: '文件对象ID', width: 120}
                    , {field: 'group_id', title: '文件分组ID', width: 120}
                    , {field: 'path', title: '文件路径', width: 120}
                    , {field: 'file_url', title: '文件路径', width: 120}
                    , {field: 'file_name', title: '文件名称', width: 120}
                    , {field: 'file_size', title: '文件大小', width: 120}
                    , {field: 'file_type', title: '文件类型', width: 120}
                    , {field: 'extension', title: '文件扩展名', width: 120}
            ...
            table.on('toolbar(tblFilter)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.msg('添加');
                        window.location.href = "{{ route('admin.uploadFiles.create') }}";
                        break;
                    case 'update':
                        if (data.length === 0) {
                            layer.msg('请选择一行');
                            ...
                            window.location.href = "{{ url('admin/uploadFiles/edit') }}/" + checkStatus.data[0].id;
                        }
                        break;
                    case 'delete':
                        if (data.length === 0) {
            ...
            table.on('tool(tblFilter)', function (obj) { //注：tool 是工具条事件名，tblFilter 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') {
                    ...
                    //跳转到编辑页面
                    window.location.href = "{{ url('admin/uploadFiles/edit') }}/" + data.id;
                }
            });
