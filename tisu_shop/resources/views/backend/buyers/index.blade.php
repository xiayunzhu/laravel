            ...
            <div class="layui-form-item">
                    <div class="layui-inline">
                        <div class="layui-input-inline">
                            <div class="layui-input-inline">
                                <input type="text" class="layui-input" name="nick_name" placeholder="买家昵称">
                            </div>
                        </div>
                    </div>
                    <div class="layui-inline">
...
@push('scripts')
    <script>
        ...
        function deleteRow(id, obj) {
            let url = "{{ url('admin/buyers/destroy') }}/" + id;
        ...
        function deleteRows(ids) {
            ...
            let url = "{{ url('admin/buyers/destroyBat') }}";
            let data = {ids: ids};
        ...
        layui.use(['table','form'], function () {
            ...
            table.render({
                elem: '#tbl'
                // , height: 312
                , url: "{{route('admin.buyers.list')}}"  //数据接口
                , title: "{{$title}}"
                , page: true //开启分页
                ...
                , cols: [[ //表头
                    ...
                    , {field: 'open_id', title: 'open_id', width: 220}
                    , {field: 'phone', title: '手机号', width: 180}
                    // , {field: 'union_id', title: ' unionId', width: 120}
                    , {field: 'nick_name', title: '微信昵称', width: 120}
                    , {field: 'avatar_url', title: '头像链接', width: 120}
                    ...
                    // , {field: 'address_id', title: '地址ID', width: 120}
            ...
            table.on('toolbar(tblFilter)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.msg('添加');
                        window.location.href = "{{ route('admin.buyers.create') }}";
                        break;
                    case 'update':
                        if (data.length === 0) {
                            layer.msg('请选择一行');
                            ...
                            window.location.href = "{{ url('admin/buyers/edit') }}/" + checkStatus.data[0].id;
                        }
                        break;
                    case 'delete':
                        if (data.length === 0) {
            ...
            table.on('tool(tblFilter)', function (obj) { //注：tool 是工具条事件名，tblFilter 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') {
                    ...
                    //跳转到编辑页面
                    window.location.href = "{{ url('admin/buyers/edit') }}/" + data.id;
                }
            });
