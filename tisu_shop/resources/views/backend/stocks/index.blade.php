...
@push('scripts')
    <script>
        ...
        function deleteRow(id, obj) {
            let url = "{{ url('admin/stocks/destroy') }}/" + id;
        ...
        function deleteRows(ids) {
            ...
            let url = "{{ url('admin/stocks/destroyBat') }}";
            let data = {ids: ids};
        ...
        layui.use(['table','form'], function () {
            ...
            table.render({
                elem: '#tbl'
                // , height: 312
                , url: "{{route('admin.stocks.list')}}"  //数据接口
                , title: "{{$title}}"
                , page: true //开启分页
                ...
                , cols: [[ //表头
                    ...
                    , {field: 'sku_code', title: '系统规格编码', width: 120}
                    , {field: 'item_code', title: '商品编码', width: 120}
                    , {field: 'quantity', title: '实际库存', width: 100}
                    , {field: 'available', title: '可用库存', width: 100}
                    , {field: 'modified', title: '修改时间', width: 160}
                    , {field: 'oln_item_id', title: '线上商品编号，即 B2C 推送到 ERP 中的 itemID', width: 120}
                    , {field: 'oln_sku_id', title: '线上规格编号', width: 120}
                    , {field: 'storage_code', title: '仓库编码', width: 120}
                    , {field: 'storage_name', title: '仓库名称', width: 120}
            ...
            table.on('toolbar(tblFilter)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.msg('添加');
                        window.location.href = "{{ route('admin.stocks.create') }}";
                        break;
                    case 'update':
                        if (data.length === 0) {
                            layer.msg('请选择一行');
                            ...
                            window.location.href = "{{ url('admin/stocks/edit') }}/" + checkStatus.data[0].id;
                        }
                        break;
                    case 'delete':
                        if (data.length === 0) {
            ...
            table.on('tool(tblFilter)', function (obj) { //注：tool 是工具条事件名，tblFilter 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') {
                    ...
                    //跳转到编辑页面
                    window.location.href = "{{ url('admin/stocks/edit') }}/" + data.id;
                }
            });
