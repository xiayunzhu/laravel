                ...
                <div class="layui-field-box" style="margin-top: 30px">
                    ...
                    <div class="">
                        ...
                        <div class=" layui-col-md3" id="picMenu">
                            {{--style="float: left;border: 1px solid #ffffff;margin-top: 30px">--}}
                            <ul class="layui-nav layui-nav-tree layui-inline" lay-filter="demo"
                                style="margin-right: 10px;">
                                <li class="layui-nav-item layui-nav-itemed">
                                    <dl class="layui-nav-child" style="background-color: #ffffff">
                                        <dd style="background-color: #ffffff;" onmouseover="this.style.cursor='hand'" id="group_-1" ><a href="{{url('admin/uploadFiles/picWindow?group_id=-1')}}" style="color: #000000;cursor:pointer;">全部</a>
                                        </dd>
                                        <dd style="background-color: #ffffff;" onmouseover="this.style.cursor='hand'" id="group_0" ><a href="{{url('admin/uploadFiles/picWindow?group_id=0')}}"  style="color: #000000;cursor:pointer;">未分组</a></dd>
                                        @if(!empty($groups))
                                            @foreach($groups as $val=>$label)
                                                <dd id="group_{{$label->id}}" style="background-color: #ffffff;cursor:pointer;">
                                                    <a  href="{{url('admin/uploadFiles/picWindow?group_id').'='.$label->id}}"
                                                        style="color: #000000;display: inline;cursor:pointer;">
                                                        <label style="color: #0C0C0C">{{$label->group_name}}</label>

                                                    </a>
                                                    <i class="layui-icon layui-icon-edit" style="color: #0C0C0C" onclick="editGroup({{$label->id}})"></i>
                                                    <i class="layui-icon layui-icon-delete" style="color: #0C0C0C"  onclick="delGroup({{$label->id}})"></i>
                                                </dd>
                                            @endforeach
                                        @endif
                                        <dd style="background-color: #ffffff;"><a href="javascript:;" id="add_group" style="color: #53bdff">新增分组+</a></dd>
                                    </dl>
                                </li>
                            </ul>
                        </div>
                                    ...
                                    @if(!empty($uploadFile))
                                        @foreach($uploadFile as $val=>$label)
                                            <li class="ng-scope" id="ng-scope_{{$label->id}}"
                                                ...
                                                data-file-path="{{ storage_image_url($label->file_url) }}"  path="{{$label->file_url}}">
                                                <div class="img-cover"
            ...
            laypage.render({
                ...
                jump: function(obj, first){
                    ...
                    if(!first) {
                        ...
                        window.location.href="{{ url("admin/uploadFiles/picWindow?")}}"+"page="+curr+tmp;
                    }
