...
@push('scripts')
    <script>
        ...
        function deleteRow(id, obj) {
            let url = "{{ url('admin/favorites/destroy') }}/" + id;
        ...
        function deleteRows(ids) {
            ...
            let url = "{{ url('admin/favorites/destroyBat') }}";
            let data = {ids: ids};
        ...
        layui.use(['table', 'form'], function () {
            ...
            table.render({
                elem: '#tbl'
                // , height: 312
                , url: "{{route('admin.favorites.list')}}"  //数据接口
                , title: "{{$title}}"
                ...
                , cols: [[ //表头
                    ...
                    , {
                        field: 'goods_id', title: '商品', sort: true, templet: function (row) {
                            return row.goods ? row.goods.name : '';
                        }
                    }
                    , {
                        field: 'buyer_id', title: '买家名', sort: true, templet: function (row) {
                            return row.user ? row.user.name : '';
                        }
                    }
                    , {
                        field: 'shop_id', title: '店铺名', sort: true, templet: function (row) {
            ...
            table.on('toolbar(tblFilter)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.msg('添加');
                        window.location.href = "{{ route('admin.favorites.create') }}";
                        break;
                    case 'update':
                        if (data.length === 0) {
                            layer.msg('请选择一行');
                            ...
                            window.location.href = "{{ url('admin/favorites/edit') }}/" + checkStatus.data[0].id;
                        }
                        break;
                    case 'delete':
                        if (data.length === 0) {
            ...
            table.on('tool(tblFilter)', function (obj) { //注：tool 是工具条事件名，tblFilter 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') {
                    ...
                    //跳转到编辑页面
                    window.location.href = "{{ url('admin/favorites/edit') }}/" + data.id;
                }
            });
