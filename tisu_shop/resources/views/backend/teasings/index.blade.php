            ...
            <div class="layui-form-item">
                <div class="layui-inline">
                    <div class="layui-input-inline">
                        <div class="layui-input-inline">
                            <input type="text" class="layui-input" name="user" placeholder="用户名字">
                        </div>
                    </div>
                </div>
                <div class="layui-inline">
...
@push('scripts')
    <script>
        ...
        function deleteRow(id, obj) {
            let url = "{{ url('admin/teasings/destroy') }}/" + id;
        ...
        function deleteRows(ids) {
            ...
            let url = "{{ url('admin/teasings/destroyBat') }}";
            let data = {ids: ids};
        ...
        layui.use(['table', 'form'], function () {
            ...
            table.render({
                elem: '#tbl'
                // , height: 312
                , url: "{{route('admin.teasings.list')}}"  //数据接口
                , title: "{{$title}}"
                , page: true //开启分页
                , toolbar: 'toolsBar' //开启工具栏，此处显示默认图标，可以自定义模板，详见文档
                // , totalRow: true //开启合计行
                , cols: [[ //表头
                    ...
                    ,{field:'user',title:'用户',templet:function (row) {
            ...
            table.on('toolbar(tblFilter)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.msg('添加');
                        window.location.href = "{{ route('admin.teasings.create') }}";
                        break;
                    case 'update':
                        if (data.length === 0) {
                            layer.msg('请选择一行');
                            ...
                            window.location.href = "{{ url('admin/teasings/edit') }}/" + checkStatus.data[0].id;
                        }
                        break;
                    case 'delete':
                        if (data.length === 0) {
            ...
            table.on('tool(tblFilter)', function (obj) { //注：tool 是工具条事件名，tblFilter 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') {
                    ...
                    //跳转到编辑页面
                    window.location.href = "{{ url('admin/teasings/detail') }}?id=" + data.id;
                  {{--window.location.href = "{{ url('admin/teasings/edit') }}/" + data.id;--}}
                }
            });
