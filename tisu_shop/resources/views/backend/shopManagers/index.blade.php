...
@push('scripts')
    <script>
        ...
        function deleteRow(id, obj) {
            let url = "{{ url('admin/shopManagers/destroy') }}/" + id;
        ...
        function deleteRows(ids) {
            ...
            let url = "{{ url('admin/shopManagers/destroyBat') }}";
            let data = {ids: ids};
        ...
        layui.use(['table','form'], function () {
            ...
            table.render({
                elem: '#tbl'
                // , height: 312
                , url: "{{route('admin.shopManagers.list')}}"  //数据接口
                , title: "{{$title}}"
                , page: true //开启分页
                ...
                , cols: [[ //表头
                    ...
                    , {field: 'shop_id', title: 'SHOP_ID', width: 120}
                    , {field: 'user_id', title: 'USER_ID', width: 120}
            ...
            table.on('toolbar(tblFilter)', function (obj) {
                ...
                switch (obj.event) {
                    case 'add':
                        layer.msg('添加');
                        window.location.href = "{{ route('admin.shopManagers.create') }}";
                        break;
                    case 'update':
                        if (data.length === 0) {
                            layer.msg('请选择一行');
                            ...
                            window.location.href = "{{ url('admin/shopManagers/edit') }}/" + checkStatus.data[0].id;
                        }
                        break;
                    case 'delete':
                        if (data.length === 0) {
            ...
            table.on('tool(tblFilter)', function (obj) { //注：tool 是工具条事件名，tblFilter 是 table 原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'detail') {
                    ...
                    //跳转到编辑页面
                    window.location.href = "{{ url('admin/shopManagers/edit') }}/" + data.id;
                }
            });
